...
def encrypt_image(image_path, key):
    ...
    try:
        ...
        encrypted_data = bytearray([(byte + key) % 256 for byte in image_data])

        # Save the encrypted data as a file in the same directory
        encrypted_file_path = os.path.join(os.path.dirname(image_path), os.path.basename(image_path) + ".enc")
...
def decrypt_image(encrypted_file_path, key):
    ...
    try:
        ...
        decrypted_data = bytearray([(byte - key) % 256 for byte in encrypted_data])
...
def select_encrypted_file():
    """Opens a file dialog to select an encrypted .enc file."""
...
def perform_encryption():
    ...
    if encrypted_path:
        messagebox.showinfo("Success", f"Image encrypted successfully! Saved as: {encrypted_path}")
...
def perform_decryption():
    ...
    if decrypted_path:
        messagebox.showinfo("Success", f"Image decrypted successfully! Saved as: {decrypted_path}")
